...
num :: Parser Integer
num = maybe id (const negate) <$> optional (char '-') <*> (toInteger1 <$> some digit)
    where toInteger1 = foldl' ((+) . (* 10)) 0
...
oper = const Add <$> char '+' <|> const Mul <$> char '*'
...
parseExpr = eatSpace *>
            ((buildOp <$> nonOp <*> (eatSpace *> oper) <*> parseExpr) <|> nonOp)
